                  ...
                  {r.matches[0] && (
                    <div className="mt-2 text-sm text-slate-600">
                      ...
                      {r.matches[0].guessedAmount != null && (() => {
                        const isDKK =
                          r.matches[0].guessedCurrency === null ||
                          r.matches[0].guessedCurrency === "DKK";
                        const matches =
                          isDKK && Math.abs(r.matches[0].guessedAmount - Math.abs(r.amount)) <= 1;
                        return (
                          <span className={matches ? " text-emerald-700 font-medium" : " text-slate-500"}>
                            {" "}
                            · gættet beløb{" "}
                            {isDKK
                              ? formatDKK(r.matches[0].guessedAmount)
                              : `${r.matches[0].guessedAmount} ${r.matches[0].guessedCurrency} (ikke DKK)`}
                            {matches && " ✓ matcher"}
                          </span>
                        );
                      })()}
